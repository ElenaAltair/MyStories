...
object ShareHelperPlot {
    fun sharePlot(plot: PlotEntity2, listName: String, context: Context): Intent {
        val intent = Intent(Intent.ACTION_SEND)
        intent.type = "text/plane"
        intent.apply {
            putExtra(Intent.EXTRA_TEXT, makeShareText(plot, listName, context))
        }
        return intent
    }

    fun makeShareText(plot: PlotEntity2, listName: String, context: Context): String {
