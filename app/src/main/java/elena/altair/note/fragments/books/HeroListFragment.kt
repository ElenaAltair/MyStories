...
import elena.altair.note.activities.MainActivity.Companion.HERO_LIST_FRAGMENT
...
@AndroidEntryPoint
class HeroListFragment : BaseFragment(), HeroAdapter.Listener, BackPressed {
    ...
    override fun onClickNew() {
        ...
        val intent = Intent(activity, NewHeroActivity::class.java).apply {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        (activity as? DialogsAndOtherFunctions)?.viewButtons(HERO_LIST_FRAGMENT)
    ...
    private fun initRcView() = with(binding) {
        ...
        rcViewHeroes.layoutManager = getLayoutManager()
    ...
    private fun observer() {
        ...
        if (book?.id != null) {
            mainViewModel.allHeroes(book?.id!!).observe(viewLifecycleOwner) {
    ...
    override fun editItem(hero: HeroEntity2) {
        val intent = Intent(activity, NewHeroActivity::class.java).apply {
            putExtra(NEW_NOTE_KEY, hero)
            putExtra(TITLE_BOOK_KEY, book)
        }
        editLauncher.launch(intent)
    }
