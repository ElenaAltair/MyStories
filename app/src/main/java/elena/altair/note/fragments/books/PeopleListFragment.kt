...
@AndroidEntryPoint
class PeopleListFragment : BaseFragment(), PeopleAdapter.Listener, BackPressed {
    ...
    override fun onClickNew() {
        ...
        val intent = Intent(activity, NewPeopleActivity::class.java).apply {
    ...
    private fun initRcView() = with(binding) {
        ...
        rcViewPeople.layoutManager = getLayoutManager()

        adapter = PeopleAdapter(
            this@PeopleListFragment,
            defPref,
    ...
    private fun observer() {
        ...
        if (book?.id != null) {
            mainViewModel.allPeoples(book?.id!!).observe(viewLifecycleOwner) {
    ...
    override fun editItem(people: PeopleEntity2) {
        val intent = Intent(activity, NewPeopleActivity::class.java).apply {
            putExtra(NEW_NOTE_KEY, people)
            putExtra(TITLE_BOOK_KEY, book)
        }
        editLauncher.launch(intent)
    }
